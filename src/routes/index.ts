...
const endpoints: Endpoint<Params, Body>[] = [
    ...
    },
    {
        ...
        callback: async (request: FastifyRequest<{ Params: Params; Body: Body }>, reply: FastifyReply) => {
            return await packeter(request.server, request.params.server_id, JSON.parse(Buffer.from(request.body.Packet, 'base64').toString('utf-8')));
        },
    },
    {
        method: 'POST',
        ...
        callback: async (request: FastifyRequest<{ Params: Params }>, reply: FastifyReply) => {
            ...
            await redis.set(`api_key:${server_id}`, createHash("sha256").update(initial_api_key).digest("hex"), { EX: 60 * 5 });
